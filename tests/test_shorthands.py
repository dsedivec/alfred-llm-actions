"""Tests for _translate_shorthands."""
...
class TestReasoning:
    def test_anthropic_auto(self):
        result = _translate_shorthands("anthropic", {"reasoning": "auto"})
        assert result["thinking"] == {"type": "enabled", "budget_tokens": 0}
        assert "reasoning" not in result

    def test_anthropic_high(self):
        result = _translate_shorthands("anthropic", {"reasoning": "high"})
        assert result["thinking"]["budget_tokens"] == 10000

    def test_anthropic_medium(self):
        result = _translate_shorthands("anthropic", {"reasoning": "medium"})
        assert result["thinking"]["budget_tokens"] == 5000

    def test_anthropic_low(self):
        result = _translate_shorthands("anthropic", {"reasoning": "low"})
        assert result["thinking"]["budget_tokens"] == 2000

    def test_openai_auto(self):
        result = _translate_shorthands("openai", {"reasoning": "auto"})
        assert result["reasoning_effort"] == "high"

    def test_openai_medium(self):
        result = _translate_shorthands("openai", {"reasoning": "medium"})
        assert result["reasoning_effort"] == "medium"

    def test_openrouter_auto(self):
        result = _translate_shorthands("openrouter", {"reasoning": "auto"})
        assert result["reasoning_effort"] == "high"

    def test_openrouter_low(self):
        result = _translate_shorthands("openrouter", {"reasoning": "low"})
        assert result["reasoning_effort"] == "low"

    def test_gemini_auto(self):
        result = _translate_shorthands("gemini", {"reasoning": "auto"})
        assert result["thinking_config"] == {"thinking_budget": -1}

    def test_gemini_high(self):
        result = _translate_shorthands("gemini", {"reasoning": "high"})
        assert result["thinking_config"]["thinking_budget"] == 10000

    def test_gemini_medium(self):
        result = _translate_shorthands("gemini", {"reasoning": "medium"})
...
class TestWebSearch:
    ...
    def test_anthropic(self):
        result = _translate_shorthands("anthropic", {"web_search": True})
        assert {"type": "web_search_20250305"} in result["tools"]

    def test_gemini(self):
        result = _translate_shorthands("gemini", {"web_search": True})
        assert {"google_search": {}} in result["tools"]

    def test_openrouter(self):
        result = _translate_shorthands("openrouter", {"web_search": True})
        assert "web" in result["plugins"]

    def test_false_is_noop(self):
        result = _translate_shorthands("openai", {"web_search": False})
...
class TestCombined:
    ...
    def test_raw_params_passthrough(self):
        result = _translate_shorthands("openai", {"temperature": 0.5})
    ...
    def test_no_shorthands(self):
        result = _translate_shorthands("openai", {"temperature": 0.7, "top_p": 0.9})
